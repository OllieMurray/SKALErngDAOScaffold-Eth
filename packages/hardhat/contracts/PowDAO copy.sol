...
contract PowDAOTEMP is Ownable{
    // GLOBAL CONSTANTS
    // ***************
    ...
    */
        ...
        returns (bool)
    {
        ...
        require(
            proposals[proposalId].flags[1] == false,
        ...
        if (prop.flags[4] == true) {
            ...
            if (prop.yesVotes > prop.noVotes) {
                ...
                prop.flags[1] = true;
                prop.flags[2] = true;
            }  else if (prop.yesVotes > prop.noVotes){
                //perform tiebreak
                if (tiebreak>0){
                    members[prop.proposer] = Member(1, 0, true, 0);
                    prop.flags[1] = true;
    ...
    function submitVote(uint256 proposalId, uint8 uintVote) public onlyMember {
        ...
        require(
            getCurrentTime() >= prop.startingTime,
            "voting period has not started"
        );
        require(
    ...
    function setMessage(string memory _string) public {
        greet=_string;
    }
    ...
    function randomOutCome() public view returns (uint8){
        bytes32 _addr32 = getRandom();
        bytes memory _addr = toBytes(_addr32);
        uint256 random = toUint256(_addr, 0);
        uint8 digit = uint8(random % 10);
        uint8 randOutcome;
        if (digit < 5){
            randOutcome = 0;
        }else{
