...
const targetNetwork = NETWORKS.skaleTestNodePortland; // <------- select your target frontend network (localhost, rinkeby, xdai, mainnet)
//NETWORKS.localhost;
  ...
  if (submitProposalEvents) {
    for (let i = 0; i < submitProposalEvents.length; i++) {
      for (let j = 0; j < processedProposalEvents.length; j++) {
        if (
          parseInt(submitProposalEvents[i].args["proposalId"]) ==
          parseInt(processedProposalEvents[j].args["proposalId"])
        ) {
